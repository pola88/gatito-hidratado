...
async function readWaterState(): Promise<{ glasses: number; goal: number; glassVolumeMl: number }> {
  try {
    const raw = await AsyncStorage.getItem(STORAGE_KEY)
    if (!raw) return { glasses: 0, goal: 8, glassVolumeMl: 250 }
    ...
    return { glasses: Math.floor(totalMl / glassVolumeMl), goal: state.today.goal ?? 8, glassVolumeMl }
  } catch {
    return { glasses: 0, goal: 8, glassVolumeMl: 250 }
  }
}
...
registerWidgetTaskHandler(async ({ widgetAction, clickAction, renderWidget }) => {
  ...
  if (widgetAction === 'WIDGET_CLICK' && clickAction === 'ADD_WATER') {
    const { glassVolumeMl } = await readWaterState()
    await addWaterToStorage(glassVolumeMl)
  }

  const { glasses, goal } = await readWaterState()
  renderWidget(React.createElement(WaterWidget, { glasses, goal }))
})
